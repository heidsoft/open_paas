...
# App Engine信息 #
##################
ENGINE_APP_INIT_URL = SimpleLazyObject(lambda: '%s/v1/apps/init' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))
ENGINE_APP_RELEASE_URL = SimpleLazyObject(lambda: '%s/v1/apps/{app_code}/releases' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))
ENGINE_APP_EVENT_LOGS_URL = SimpleLazyObject(lambda: '%s/v1/apps/{app_code}/events/{event_id}/logs' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))
ENGINE_APP_INFO_URL = SimpleLazyObject(lambda: '%s/v1/apps/{app_code}/info' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))
ENGINE_AGENT_HEALTHZ_URL = SimpleLazyObject(lambda: '%s/v1/agents/healthz' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))
ENGINE_SERVICE_STATUS_URL = SimpleLazyObject(lambda: '%s/v1/services/{category}/check/?server_id={server_id}' % getattr(sys.modules['settings'], 'ENGINE_HOST', 'http://127.0.0.1:8000'))


##################
# 应用访问链接 #
##################
APP_TEST_URL = SimpleLazyObject(lambda: 'http://%s/t/{app_code}/' % getattr(getattr(sys.modules['django.conf'], 'settings'), 'PAAS_DOMAIN'))
APP_PROD_URL = SimpleLazyObject(lambda: 'http://%s/o/{app_code}/' % getattr(getattr(sys.modules['django.conf'], 'settings'), 'PAAS_DOMAIN'))
